...
static std::wstring Utf8BytesToWideString(const char* input, size_t size)
{
    ...
    return MusicPlayerLibrary::LocaleConverter::GetUtf16StringFromUtf8String(std::string(input, size));
}
    ...
    public:
        ...
        uint32_t Read(void* buffer, uint32_t count) override
        {
            ...
            if (source_position < audio_offset_
                || source_position > static_cast<uint64_t>((std::numeric_limits<int64_t>::max)()))
            {
                NATIVE_TRACE("err: ncm decrypted audio source position overflow\n");
                return 0;
            }

            const uint64_t actual_position =
                source_file_->Seek(static_cast<int64_t>(source_position), MusicPlayerLibrary::FileSeekOrigin::Begin);
            if (actual_position == MusicPlayerLibrary::SeekFailure || actual_position != source_position)
            {
...
void MusicPlayerLibrary::NcmDecryptor::SeekSource(uint64_t offset, const char* message)
{
    ...
    const uint64_t position = m_sourceFile.Seek(static_cast<int64_t>(offset), FileSeekOrigin::Begin);
...
void MusicPlayerLibrary::NcmDecryptor::ReadSourceExact(void* buffer, uint32_t count, const char* message)
{
    ...
    {
        const uint32_t bytes_read = m_sourceFile.Read(bytes + total_read, count - total_read);
        if (bytes_read == 0)
            throw std::runtime_error(message);
...
uint64_t MusicPlayerLibrary::NcmDecryptor::GetAudioOffset()
{
    ...
    constexpr uint64_t AudioHeaderSize = 13;
    if (AudioHeaderSize > m_fileLength - m_offset
        || static_cast<uint64_t>(dataLen) > m_fileLength - m_offset - AudioHeaderSize)
        throw std::runtime_error("audio offset out of range");
    m_offset += static_cast<uint64_t>(dataLen) + AudioHeaderSize;
    return m_offset;
}
